...
//定义小工具类AshuPostViews    
class Faith_2 extends WP_Widget{   
    ...
    parent::__construct(
        ...
        // 小工具ID
        'Faith_firstpic_posts',
 
        // 小工具名称
        __('Faith-分类首篇图片列表', 'Faith_2' ),
 
        // 小工具选项
        array (
            'description' => __( '按照选定的分类目录显示文章并首篇文章显示图片', 'Faith_2' )
        )
    ...
    function widget($args,$instance){  
        ...
        echo '<ul>';   
        $this->faith_2_get_posts($showPosts,$catId); //使用ashu_get_hotpost函数获取文章   
        echo '</ul>';   
        echo $after_widget;    
        //这是控制小工具前台显示的函数   
    }   
     ...
     //$showposts参数为显示文章的数量   
    function faith_2_get_posts($showposts,$catId){   
         if (have_posts()) : 
                ...
                while (have_posts()) : the_post(); 
                 ?>
                        ...
                        <li class="thumb">
                            ...
                            <p><?php echo mb_strimwidth(get_the_content(), 0, 160,"..."); ?></p>
                        </li>
                <?php endwhile; wp_reset_query();
                        $showposts = $showposts -1;
                        query_posts('cat=' . $catId. '&showposts='.$showposts.'&offset=1'); 
                        while (have_posts()) : the_post(); ?>
...
function Faith_2(){   
    //注册小工具   
    register_widget('Faith_2');   
}   
//widges_init，小工具初始化的时候执行AshuPostViews函数，   
add_action('widgets_init','Faith_2');   
